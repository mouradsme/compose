...
import { singinAdminServer } from '@/lib/auth/login';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    ...
    try {
      await signinSchema.validate(admin, { abortEarly: true });
    } catch (err) {
    ...
    try {
      response = await singinAdminServer(admin);
      res.setHeader(
        'Set-Cookie',
        `${ACCESS_TOKEN_NAME}=${JSON.stringify(response.data)}; SameSite=Strict; Path=/`
      );
