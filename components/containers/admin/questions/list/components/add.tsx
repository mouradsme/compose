import { QuestionCreate } from '@/common/types/common';
import { useTranslation } from 'next-i18next';
import { useReducer } from 'react';

const initialState: QuestionCreate = {
  content: [''],
  suggestedAnswers: [{ answer: '', isCorrect: false }],
};
...
const addQuestionReducer = (state: QuestionCreate, action: Action) => {
  switch (action.type) {
...
const AddQuestion = ({ onSubmit, onCancel }: AddQuestionProps) => {
  const { t } = useTranslation('quizzes');
  const [state, dispatch] = useReducer(addQuestionReducer, initialState);

  return (
    <div className="p-5">
      <div className="p-5">
        <div className="p-5">
          <h4 className="text-xl font-semibold  mb-1">{t('createAQuizQuestion')}</h4>
